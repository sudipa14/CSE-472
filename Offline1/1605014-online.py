...
def adaboost(dataframe,k):
    ...
    x_train_or, x_test_or, y_train_or, y_test_or = train_test_split(x_train_first, y_target, test_size=0.1, random_state=5)
    ...
    while i<=k:
        ...
        weight_sample=logistic_reg(x_given_sampled,y_target_sampled,theta,.01,5000)
...
theta = gradient_descent(x_test, y_test, theta, 0.01,1000)
# print(theta)
...
df_online=pd.read_csv("online.txt",header = None)
df_online=df_online.replace(' <=50K',-1)
df_online=df_online.replace(' >50K',1)
print(df_online.head())
dtypes_online=df_online.dtypes
df_online=df_online.replace(' ?', np.nan)
# # print(df_2.isin([' ?']).any())
nan_values = df_online.isna()
nan_columns = nan_values.any()
columns_with_nan = df_online.columns[nan_columns].tolist()
print(columns_with_nan)
print(df_online.dtypes)
for i in range(len(columns_with_nan)):
    #     print(dtypes_df2[i])
    if(dtypes_online[columns_with_nan[i]]=="object"):
        df_online[columns_with_nan[i]].fillna(df_online[columns_with_nan[i]].mode()[0], inplace=True)

    else:
        mean_value=df_online[columns_with_nan[i]].mean()
        df_online.fillna(value=mean_value, inplace=True)
ob_list=[1,3,5,6,7,8,9,13]
for i in range(len(ob_list)):
    df_online[ob_list] = df_online[ob_list].astype('category')
    temp =df_online[ob_list[i]].cat.codes
    del df_online[ob_list[i]]
    df_online.insert(loc=ob_list[i], column=ob_list[i], value=temp)
    
print(df_online)
x_online= df_online.values #returns a numpy array
min_max_scaler = preprocessing.MinMaxScaler()
x_scaled_online= min_max_scaler.fit_transform(x_online)
df_online = pd.DataFrame(x_scaled_online)
print(df_online.head())
y_online=df_online[14]
y_online=y_online.replace(0.0,-1.0)


x_online=df_online.drop(df_online.columns[14], axis=1)
ones = np.ones(x_online.shape[0])
df_new_online= pd.DataFrame()
df_new_online.insert(0,0,ones)
i=0
while i <(len(x_online.columns)):
    df_new_online.insert(i+1,i+1,x_online[i])
    i=i+1
x_online=df_new_online.copy(deep=True)
print(x_online.head())
theta_online= np.zeros((len(x_online.columns),1))

x_traino, x_testo, y_traino, y_testo= train_test_split(x_online, y_online, test_size=0.1, random_state=2)

y_testo = y_testo.to_numpy()
y_traino = y_traino.to_numpy()
y_traino=np.reshape(y_traino,(len(y_traino),1))
y_testo=np.reshape(y_testo,(len(y_testo),1))

theta_online = gradient_descent(x_traino, y_traino, theta_online, 0.01,10000)
# print(theta)
Y_pred =predict(x_testo,theta_online)
perf_measure(y_testo,Y_pred)
# gradient decent done
...
# adaboost
final_list_on=list()
df_new_on=x_online.copy(deep=True)
df_new_on.insert(len(x_online.columns),len(x_online.columns),y_online)

y_test_or2,x_test_or2,list_w2,list_z2=adaboost(df_new_on,5)
for i in range(len(list_z2)):
    Y_pred_ada2=predict(x_test_or2,list_w2[i])
    final_list_on.append(Y_pred_ada2*list_z2[i])
res2=np.sum(final_list_on,axis=0)
res2=np.where(res2>=0, 1,-1)
accuracy(res2,y_test_or2)
